...
pub fn download(config_path: &str, output_path: &str) {
    ...
    let _ = &config.fabric.mods.par_iter().for_each(move |modid| {
        ...
        let _ = &data.versions.par_iter().for_each(|version| {
            ...
            let data = match serde_json::from_str::<super::structs::modrinth_version::Root>(res.as_str()) {
                Ok(data) => data,
                Err(_) => return
            };
        ...
        match download_file_blocking(&blocking, &download_url, output_path) {
...
fn download_file_blocking(client: &reqwest::blocking::Client, url: &str, output_path: &str) -> io::Result<()> {
